      ...
      {isOpen && (
        <div className="absolute top-14 right-0 z-50 w-64 p-2 rounded-3xl bg-zinc-900 border border-white/10 shadow-2xl backdrop-blur-xl">
          ...
          <div className="flex flex-col gap-1">
            {seasons.filter(s => s.season_number > 0).map((s) => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-bold text-lg font-serif">시즌 {s.season_number}</span>
            ...
            <button className="w-full px-5 py-4 text-left text-zinc-500 hover:text-zinc-300 transition-colors text-sm font-serif">
              전체 회차 표시
            </button>
          </div>
        </div>
